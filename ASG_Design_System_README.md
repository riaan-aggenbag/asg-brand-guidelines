...
**Static-output canon (folded into THEME-SPEC §8/§9.1).** All HTML output from here is: clean semantic static HTML + vanilla CSS custom properties; **components delimited by `<!-- component: name -->` … `<!-- /component -->` on every HTML output in every chat (standing per-output rule)**; **no page-builder markup, no WordPress conventions/shortcodes, no React/Vue/framework, no build-tooling assumptions**; **images at fixed aspect ratios with dimensions in markup** against Cloudflare Images placeholders (never final assets) — the **ratified locked set** is hero 16:9 · gallery tall 2:3 · gallery square 1:1 · room 3:2 · sibling/Reserve 3:2 · thumbnail 1:1 (sources in `design-tokens.json → image.aspectRatios`; gallery tall is coupled to the current gallery grid); **JavaScript minimal and dependency-free** — behaviour described in a comment where interactivity is genuinely needed. Brand rules (colour, type, one brass CTA per screen, no emoji, no bubbly buttons) remain fully in force.

**The rates block's default state is the enquire-fallback, not the exception.** Rate data renders only once a **rate row is certified in the database**; at launch no rows are certified, so **every lodge on every site renders the fallback** — "From R xx,xxx — enquire" / "From $x,xxx — enquire", one currency-agnostic component state accepting either symbol. Design it as the **polished primary state** (equal design quality); the full rate grid is the **enhanced state a lodge graduates into** on certification. This makes the fallback the launch state site-wide, and removes any dependency on native ZAR rates being in hand before the Tier 2 clones ship.
